...
public class ProductDAO {
    ...
    public String addOrder(User user, float total) throws SQLException, ClassNotFoundException {
        ...
        try {
            ...
            if (cn != null) {
                ...
                String sql = "insert into [dbo].[tblOders]([orderId],[userAccount],[totalPrice],[date],[fullName],[phone],[email],[address],[statusID]) values (?,?,?,?,?,?,?,?,1)";
                pst = cn.prepareStatement(sql);
    ...
    private int checkOrder(List<History> list, String orderid) {
        for (int i = 0; i < list.size(); i++) {
            ...
            if (cn != null) {
                ...
                while (rs.next()) {
                    ...
                    if (result == null) {
                        ...
                        his = new History(date, statusName, userName, userAccount, orderId, null);
                        Product tmp = new Product(proID, proName, price, "", "", image, quanity, true, "", "");
                        his.add(orderId, tmp);
                        result.add(his);
                    } else {
                        int indexHis = checkOrder(result, orderId);
                        if (indexHis < 0) {
                            his = new History(date, statusName, userName, userAccount, orderId, null);
                            Product tmp = new Product(proID, proName, price, "", "", image, quanity, true, "", "");
                            his.add(orderId, tmp);
    ...
    public List<Status> getStatus() throws ClassNotFoundException, SQLException {
        ...
        try {
            cn = MyConnection.getConnection();
            if (cn != null) {
                String sql = "select * from [dbo].[tblStatus]";
                pst = cn.prepareStatement(sql);
                rs = pst.executeQuery();
                while (rs.next()) {
                    int cateID = rs.getInt("statusID");
                    String cateName = rs.getString("statusName");
                    if (result == null) {
                        result = new ArrayList<>();
                    }
                    result.add(new Status(cateID, cateName));
                }
            }
        } finally {
            if (rs != null) {
    ...
    public void updateOrder(String orderId, String statusId) throws ClassNotFoundException, SQLException {
        Connection cn = null;
        PreparedStatement pst = null;
        try {
            cn = MyConnection.getConnection();
            if (cn != null) {
                String sql = "UPDATE tblOders\n"
                        + "SET statusID=?\n"
                        + "WHERE orderId=?";
                pst = cn.prepareStatement(sql);
