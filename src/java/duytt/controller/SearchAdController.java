...
public class SearchAdController extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (user == null) {
				...
				if (user.getRoleID().equals("AD")) {
					...
					if(check)
					{
						...
						sql = "with tblTemp as(select ROW_NUMBER() over ( order by [createDate] asc) as num,[foodId],[foodName],[price],[desciption],[quantity],[status],[createDate],[image],[categoryId],hsd from [dbo].[tblFood] where [foodName] like N'%" + txtSearch + "%' and price >= " + minPrice + "\n"
						...
						sqlpage = "select count(foodId) as soluong from [dbo].[tblFood] where [foodName] like N'%" + txtSearch + "%'  and price >= " + minPrice + "\n"
								+ "and price <=  " + maxPrice;
			...
			log("SearchAdController: " + e.toString());
		} finally {
