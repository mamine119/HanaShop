...
public class OderController extends HttpServlet {
    ...
    private final static String ERROR = "order.jsp";
        ...
        try {
            ...
            if (userT == null) {
                ...
                if (userT.getRoleID().equals("AD")) {
                    ...
                    String sql = "";
                    if (date.equals("")) {
                        sql = "select b.foodId,[foodName],[image],b.[quanity],b.[price],[date],b.orderId,e.statusName,d.fullName,d.userAccount from [dbo].[tblFood]a,[dbo].[tblOderDetail]b,[dbo].[tblOders]c,[dbo].[tblUsers] d, [dbo].[tblStatus] e where a.foodId=b.foodId and b.orderId=c.orderId and c.userAccount=d.userAccount and c.statusID=e.statusID and c.orderId in (select c.[orderId] from [dbo].[tblFood]a,[dbo].[tblOderDetail]b,[dbo].[tblOders]c,[dbo].[tblUsers] d where a.foodId=b.foodId and b.orderId=c.orderId and c.userAccount=d.userAccount and foodName like N'%%' group by c.[orderId]) ORDER BY [date] DESC,orderId DESC";
                    } else {
                        sql = "select b.foodId,[foodName],[image],b.[quanity],b.[price],[date],b.orderId,e.statusName,d.fullName,d.userAccount from [dbo].[tblFood]a,[dbo].[tblOderDetail]b,[dbo].[tblOders]c,[dbo].[tblUsers] d, [dbo].[tblStatus] e where a.foodId=b.foodId and b.orderId=c.orderId and c.userAccount=d.userAccount and c.statusID=e.statusID and c.orderId in (select c.[orderId] from [dbo].[tblFood]a,[dbo].[tblOderDetail]b,[dbo].[tblOders]c,[dbo].[tblUsers] d where a.foodId=b.foodId and b.orderId=c.orderId and c.userAccount=d.userAccount and foodName like N'%%'and [date] = '" + date + "' group by c.[orderId]) ORDER BY [date] DESC,orderId DESC";
                    }
                    List<History> list = new ProductDAO().getHistory(sql);
                    List<Status> listStatus = (List<Status>) new ProductDAO().getStatus();
